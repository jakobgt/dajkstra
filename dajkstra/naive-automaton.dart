...
class NaiveAutomaton {
  ...
  State startStepping(Graph graph) {
    return new NodeState(graph.start, new PList(), 0, graph, new EmptyContext());
  }
...
class NodeState extends State {
  ...
  NodeState(this.currentNode, this.currentPath, this.currentCost, this.graph, this.cont);

  State step() {
    var currentFullPath = currentPath.cons(currentNode);
    return (currentNode.id == graph.end.id) ? new PathState(new ContState(cont, new Result(currentFullPath, currentCost))) : ((currentPath.any((elm) => elm.id == currentNode.id)) ? new CycleState(new ContState(cont, new Result.NoPath())) : new EdgesState(graph.adjacent(currentNode), new Result.NoPath(), currentFullPath, currentCost, graph, cont));
  }
...
class EdgesState extends State {
  ...
  EdgesState(this.edges, this.bestRes, this.currentFullPath, this.currentCost, this.graph, this.cont);

  State step() {
    return (edges.empty) ? new ContState(cont, bestRes) : new NodeState(edges.hd.dest, currentFullPath, currentCost + edges.hd.cost, graph, new EdgesContext(edges, bestRes, currentFullPath, currentCost, graph, cont));
  }
...
class EdgesContext extends Context {
  ...
  EdgesContext(this.edges, this.bestRes, this.currentFullPath, this.currentCost, this.graph, this.cont);

  State apply(Result result) => new EdgesState(edges.tl, (result.cost < bestRes.cost) ? result : bestRes, currentFullPath, currentCost, graph, cont);
